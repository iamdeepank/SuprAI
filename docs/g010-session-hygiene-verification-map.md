...
| --- | --- | --- |
| Generated session files are not accidentally committed | `.gitignore`, `rust/.gitignore` ignore `.suprai/sessions/` and `.claude/sessions/` | `git check-ignore .suprai/sessions/example.jsonl rust/.suprai/sessions/example.jsonl .claude/sessions/example.json` |
...
- Runtime-owned session state remains under ignored `.suprai/sessions/<workspace-fingerprint>/` paths.
