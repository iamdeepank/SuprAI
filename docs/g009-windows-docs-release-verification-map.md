...
$env:SUPRAI_CONFIG_HOME = Join-Path $env:TEMP "claw-smoke-config"
...
env -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN -u OPENAI_API_KEY -u XAI_API_KEY -u DASHSCOPE_API_KEY \
  SUPRAI_CONFIG_HOME="$(mktemp -d)" cargo run -p rusty-claude-cli -- --help
```
...
1. PowerShell examples in `README.md` build and run `suprai.exe` from a clean Windows checkout.
...
3. Windows smoke CI runs help/doctor/config/status without live credentials, separates native PowerShell `suprai.exe` smoke from WSL `install.sh` smoke, and archives JSON output on failure.
