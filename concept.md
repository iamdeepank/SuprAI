# Концепция проекта SuprAI

Документ фиксирует **цели**, **архитектуру** и **принципы** репозитория **SuprAI** — публичной Rust-реализации CLI-агента **`suprai`** и сопутствующих инструментов. Источник правды по кодовой базе: workspace в каталоге [`rust/`](rust/README.md); операционные сценарии — [`USAGE.md`](USAGE.md), [`how_to_run.md`](how_to_run.md) (claw-analog), бэклог идеи — [`futute.md`](futute.md).
...
1. **Основной CLI `suprai`** (`rusty-claude-cli`): полнофункциональный агент с REPL, OAuth, расширенным набором инструментов (включая bash, MCP, плагины и др.), стримингом и интеграцией с провайдерами **Anthropic**, **OpenAI-совместимыми** API и **xAI**.
2. **`suprai-analog`** — облегчённая оболочка на **том же слое API** (`api` crate): узкий, предсказуемый набор инструментов только для работы с файловой системой воркспейса, явные режимы прав, пригодность для **CI**, **скриптов** и **внешних агентов** (NDJSON).
3. **`suprai-rag-service`** — отдельный процесс: **индексация** репозитория (чанки + эмбеддинги в SQLite), **HTTP API** для семантического поиска и минимальный **веб-UI** для ручной проверки индекса.

Общая идея: дать **безопасный**, **аудируемый** и **воспроизводимый** способ вызова LLM над кодом и документацией, с путём эволюции от минимального harness до полного `suprai`.
...
| Сегмент | Задача |
|---------|--------|
| Разработчик | Ежедневная работа с кодовой базой через полный `suprai`: REPL, инструменты, сессии. |
| Автор автоматизации | Одноразовые промпты, пайплайны с `--output-format json`, встроенные агенты без bash. |
| Сопровождение / аудит | `suprai-analog` в **read-only** + пресет **audit**; явные лимиты и политика. |
...
**Принцип разделения:** тяжёлая индексация и хранение эмбеддингов **не** зашиваются в `suprai-analog`, а живут в **`suprai-rag-service`**. Агент только вызывает retrieval по HTTP — проще масштабировать, менять векторное хранилище и секреты эмбеддингов.
...
4. **Модульность** — общий `api` для провайдеров; `suprai-analog` не дублирует стек ключей RAG, только HTTP-клиент к сервису.
...
- **`rusty-claude-cli`** — основной бинарь **`suprai`**: пользовательский продукт полной мощности.
...
- **`suprai-analog`** — минимальный цикл: инструменты чтения/поиска/записи (по режиму), стриминг и JSON, TOML-конфиг, сессии, doctor, config validate, **retrieve_context** при наличии `RAG_BASE_URL` / `rag_base_url`.
- **`suprai-rag-service`** — `ingest`, `serve`, маршруты `/`, `/health`, `/v1/stats`, `/v1/query`; SQLite + OpenAI-совместимые эмбеддинги (или mock для тестов).
...
**Не входит в минимальный дизайн:** MCP, плагины, bash — это зона **полного `suprai`**.
...
## 9. Связанные концепции (не ядро SuprAI)

В **`docs/`** могут находиться переносимые заметки для **других** продуктов (например локальный vision для NestJS-приложений) — они **не** определяют обязательное поведение `suprai`, но отражают смежный интерес contributors.

---

## 10. Итоговая формулировка

**SuprAI** — это экосистема **Rust** вокруг агента **`suprai`**: полный CLI для разработчиков, **`suprai-analog`** как управляемый минимальный агент для автоматизации и **отдельный RAG-сервис** для семантического поиска по коду. Проект опирается на **явные права**, **лимиты**, **тестируемость** и **чёткие HTTP-границы** между агентом и тяжёлой индексацией.
